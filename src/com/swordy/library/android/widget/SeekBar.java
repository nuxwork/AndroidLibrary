...
public class SeekBar extends View {
	...
	protected void onCreate(Context context, AttributeSet attrs, int defStyle) {
		...
		mProgressHeight = a.getDimensionPixelSize(R.styleable.SeekBar_progressHeight,
				LayoutParams.WRAP_CONTENT);
		mThumb = a.getDrawable(R.styleable.SeekBar_thumb);
		mThumbWidth = a.getDimensionPixelSize(R.styleable.SeekBar_thumbWidth,
				LayoutParams.WRAP_CONTENT);
		mThumbHeight = a.getDimensionPixelSize(R.styleable.SeekBar_thumbHeight,
				LayoutParams.WRAP_CONTENT);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		// return procHorizontalTouchEvent(event);
		// }else{
	...
	public boolean procVerticalTouchEvent(MotionEvent event) {
		...
		if (mOrientation == VERTICAL) {
			...
			setProgress((int) (dis / mProgressRect.height() * mMax), true);
			...
			// dis = mProgressRect.width() - dis;

			setProgress((int) (dis / mProgressRect.width() * mMax), true);
		}
	...
	private void drawHorizental(Canvas canvas) {
		...
		if (mProgressDrawable instanceof LayerDrawable) {
			Drawable background = ((LayerDrawable) mProgressDrawable)
					.findDrawableByLayerId(android.R.id.background);
			Drawable progress = ((LayerDrawable) mProgressDrawable)
			...
			if (progress != null) {
				halfSize = mProgressHeight / 2;
				progress.setBounds(rect.left, centerY - halfSize, rect.left + progressSpec,
						centerY + halfSize);
		...
		if (mThumb != null) {
			...
			mThumb.setBounds(offset - halfThumbWidth, centerY - halfSize, offset + halfThumbWidth,
					centerY + halfSize);
			mThumb.draw(canvas);
		}
	}
	...
	private void drawVertical(Canvas canvas) {
		...
		if (mProgressDrawable instanceof LayerDrawable) {
			...
			if (progress != null) {
				halfSize = mProgressHeight / 2;
				progress.setBounds(centerX - halfSize, rect.bottom - progressSpec, centerX
						+ halfSize, rect.bottom);
				progress.draw(canvas);
			}
		}
		...
		if (mThumb != null) {
			...
			mThumb.setBounds(centerX - halfSize, offset - halfThumbHeight, centerX + halfSize,
					offset + halfThumbHeight);
			mThumb.draw(canvas);
		}
	}
